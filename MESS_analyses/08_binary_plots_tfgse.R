...
glypout <- raster("median_consensus_LTPthresh_tfgSe.asc")
...
ggplot(data = glypout_df, aes(y = y, x = x)) +
  geom_raster(aes(fill = factor(median_consensus_LTPthresh_tfgSe))) +
...
#save
setwd("C:/Users/Kat/Desktop/plots/MESS")
ggsave("LTP_tfgSe_zoom.eps", width = 15, height = 7, units = "cm")

#get maxent data MaxSSS
setwd("C:/Users/Kat/Desktop/ENM_Fourth_Try/kuenm_MESS/maxsss_tfgSe")
glypout <- raster("median_consensus_maxsssthresh_tfgSe.asc")
proj4string(glypout) <- CRS("+proj=longlat")
glypout_pts <- rasterToPoints(glypout)
glypout_df <- data.frame(glypout_pts)
...
ggplot(data = glypout_df, aes(y = y, x = x)) +
  geom_raster(aes(fill = factor(median_consensus_maxsssthresh_tfgSe))) +
  scale_fill_manual(values=c('grey90','plum3')) +
  geom_path(
    data = world_df,
    aes(long, lat, group = group),
...
#save
setwd("C:/Users/Kat/Desktop/plots/MESS")
ggsave("MaxSSS_tfgSe_zoom.eps", width = 15, height = 7, units = "cm")
