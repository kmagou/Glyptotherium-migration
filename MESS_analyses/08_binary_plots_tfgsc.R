...
#get maxent data LTP
setwd("C:/Users/Kat/Desktop/ENM_Fourth_Try/kuenm_MESS/LTP_tfgSc")
glypout <- raster("median_consensus_LTPthresh_tfgSc.asc")
...
ggplot(data = glypout_df, aes(y = y, x = x)) +
  geom_raster(aes(fill = factor(median_consensus_LTPthresh_tfgSc))) +
...
setwd("C:/Users/Kat/Desktop/ENM_Fourth_Try/kuenm_MESS/maxsss_tfgSc")
glypout <- raster("median_consensus_maxsssthresh_tfgSc.asc")
proj4string(glypout) <- CRS("+proj=longlat")
glypout_pts <- rasterToPoints(glypout)
glypout_df <- data.frame(glypout_pts)
...
ggplot(data = glypout_df, aes(y = y, x = x)) +
  geom_raster(aes(fill = factor(median_consensus_maxsssthresh_tfgSc))) +
  scale_fill_manual(values=c('grey90','plum3')) +
  geom_path(
    data = world_df,
    aes(long, lat, group = group),
